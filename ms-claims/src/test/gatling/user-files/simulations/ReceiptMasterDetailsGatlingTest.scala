...
/**
 * Performance test for the ReceiptMasterDetails entity.
 */
class ReceiptMasterDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ReceiptMasterDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all receiptMasterDetails")
            .get("/services/claims/api/receipt-master-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new receiptMasterDetails")
            .post("/services/claims/api/receipt-master-details")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "receiptNo":"SAMPLE_TEXT"
                , "depositId":"0"
                , "depositNo":"SAMPLE_TEXT"
                , "receiptReference":"SAMPLE_TEXT"
                , "receiptAmount":"0"
                , "paymentMode":"SAMPLE_TEXT"
                , "paymentDate":"2020-01-01T00:00:00.000Z"
                , "receiptNotes":"SAMPLE_TEXT"
                , "status":"SAMPLE_TEXT"
                , "createdById":"0"
                ...
                , "receiptMasterDetailsUuid":null
                , "receiptStatus":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_receiptMasterDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created receiptMasterDetails")
                .get("/services/claims${new_receiptMasterDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created receiptMasterDetails")
            .delete("/services/claims${new_receiptMasterDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
