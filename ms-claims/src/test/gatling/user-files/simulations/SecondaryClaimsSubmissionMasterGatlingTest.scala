...
/**
 * Performance test for the SecondaryClaimsSubmissionMaster entity.
 */
class SecondaryClaimsSubmissionMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SecondaryClaimsSubmissionMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all secondaryClaimsSubmissionMasters")
            .get("/services/claims/api/secondary-claims-submission-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new secondaryClaimsSubmissionMaster")
            .post("/services/claims/api/secondary-claims-submission-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "salesOrderId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_secondaryClaimsSubmissionMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created secondaryClaimsSubmissionMaster")
                .get("/services/claims${new_secondaryClaimsSubmissionMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created secondaryClaimsSubmissionMaster")
            .delete("/services/claims${new_secondaryClaimsSubmissionMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
