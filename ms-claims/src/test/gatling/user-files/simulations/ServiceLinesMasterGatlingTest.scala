...
/**
 * Performance test for the ServiceLinesMaster entity.
 */
class ServiceLinesMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ServiceLinesMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all serviceLinesMasters")
            .get("/services/claims/api/service-lines-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new serviceLinesMaster")
            .post("/services/claims/api/service-lines-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_serviceLinesMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created serviceLinesMaster")
                .get("/services/claims${new_serviceLinesMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created serviceLinesMaster")
            .delete("/services/claims${new_serviceLinesMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
