...
/**
 * Performance test for the SecondaryServiceLinesMaster entity.
 */
class SecondaryServiceLinesMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SecondaryServiceLinesMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all secondaryServiceLinesMasters")
            .get("/services/claims/api/secondary-service-lines-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new secondaryServiceLinesMaster")
            .post("/services/claims/api/secondary-service-lines-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_secondaryServiceLinesMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created secondaryServiceLinesMaster")
                .get("/services/claims${new_secondaryServiceLinesMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created secondaryServiceLinesMaster")
            .delete("/services/claims${new_secondaryServiceLinesMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
