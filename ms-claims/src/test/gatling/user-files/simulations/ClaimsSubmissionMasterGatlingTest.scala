...
/**
 * Performance test for the ClaimsSubmissionMaster entity.
 */
class ClaimsSubmissionMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ClaimsSubmissionMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all claimsSubmissionMasters")
            .get("/services/claims/api/claims-submission-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new claimsSubmissionMaster")
            .post("/services/claims/api/claims-submission-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "salesOrderId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_claimsSubmissionMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created claimsSubmissionMaster")
                .get("/services/claims${new_claimsSubmissionMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created claimsSubmissionMaster")
            .delete("/services/claims${new_claimsSubmissionMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
