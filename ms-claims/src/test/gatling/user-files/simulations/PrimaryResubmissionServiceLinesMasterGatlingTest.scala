...
/**
 * Performance test for the PrimaryResubmissionServiceLinesMaster entity.
 */
class PrimaryResubmissionServiceLinesMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PrimaryResubmissionServiceLinesMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all primaryResubmissionServiceLinesMasters")
            .get("/services/claims/api/primary-resubmission-service-lines-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new primaryResubmissionServiceLinesMaster")
            .post("/services/claims/api/primary-resubmission-service-lines-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_primaryResubmissionServiceLinesMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created primaryResubmissionServiceLinesMaster")
                .get("/services/claims${new_primaryResubmissionServiceLinesMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created primaryResubmissionServiceLinesMaster")
            .delete("/services/claims${new_primaryResubmissionServiceLinesMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
