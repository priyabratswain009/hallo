...
/**
 * Performance test for the PrimaryClaimsReSubmissionMaster entity.
 */
class PrimaryClaimsReSubmissionMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PrimaryClaimsReSubmissionMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all primaryClaimsReSubmissionMasters")
            .get("/services/claims/api/primary-claims-re-submission-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new primaryClaimsReSubmissionMaster")
            .post("/services/claims/api/primary-claims-re-submission-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "salesOrderId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_primaryClaimsReSubmissionMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created primaryClaimsReSubmissionMaster")
                .get("/services/claims${new_primaryClaimsReSubmissionMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created primaryClaimsReSubmissionMaster")
            .delete("/services/claims${new_primaryClaimsReSubmissionMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
