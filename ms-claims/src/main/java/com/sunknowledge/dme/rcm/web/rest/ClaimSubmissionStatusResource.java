...
public class ClaimSubmissionStatusResource {
    ...
    private static final String ENTITY_NAME = "claimsClaimSubmissionStatus";
    ...
    public ResponseEntity<ClaimSubmissionStatusDTO> createClaimSubmissionStatus(
        @Valid @RequestBody ClaimSubmissionStatusDTO claimSubmissionStatusDTO
        ...
        ClaimSubmissionStatusDTO result = claimSubmissionStatusService.save(claimSubmissionStatusDTO);
        return ResponseEntity
            .created(new URI("/api/claim-submission-statuses/" + result.getClaimStatusId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getClaimStatusId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<ClaimSubmissionStatusDTO> updateClaimSubmissionStatus(
        ...
        if (!claimSubmissionStatusRepository.existsById(claimStatusId)) {
            throw new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound");
        }

        ClaimSubmissionStatusDTO result = claimSubmissionStatusService.update(claimSubmissionStatusDTO);
        return ResponseEntity
            .ok()
            .headers(
                HeaderUtil.createEntityUpdateAlert(
                    ...
                    false,
                    ENTITY_NAME,
                    claimSubmissionStatusDTO.getClaimStatusId().toString()
                )
            )
            .body(result);
    }
    ...
    public ResponseEntity<ClaimSubmissionStatusDTO> partialUpdateClaimSubmissionStatus(
        @PathVariable(value = "claimStatusId", required = false) final Long claimStatusId,
        @NotNull @RequestBody ClaimSubmissionStatusDTO claimSubmissionStatusDTO
    ) throws URISyntaxException {
        ...
        Optional<ClaimSubmissionStatusDTO> result = claimSubmissionStatusService.partialUpdate(claimSubmissionStatusDTO);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, claimSubmissionStatusDTO.getClaimStatusId().toString())
        );
    }
    ...
    public ResponseEntity<List<ClaimSubmissionStatusDTO>> getAllClaimSubmissionStatuses(
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get a page of ClaimSubmissionStatuses");
        Page<ClaimSubmissionStatusDTO> page = claimSubmissionStatusService.findAll(pageable);
    ...
    public ResponseEntity<ClaimSubmissionStatusDTO> getClaimSubmissionStatus(@PathVariable Long id) {
        log.debug("REST request to get ClaimSubmissionStatus : {}", id);
        Optional<ClaimSubmissionStatusDTO> claimSubmissionStatusDTO = claimSubmissionStatusService.findOne(id);
    ...
    public ResponseEntity<Void> deleteClaimSubmissionStatus(@PathVariable Long id) {
        log.debug("REST request to delete ClaimSubmissionStatus : {}", id);
        claimSubmissionStatusService.delete(id);
        return ResponseEntity
            .noContent()
