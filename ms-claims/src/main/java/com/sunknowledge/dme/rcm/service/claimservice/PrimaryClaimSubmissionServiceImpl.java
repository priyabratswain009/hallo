...
import com.sunknowledge.dme.rcm.pojo.primary.ClaimInformation;
...
import com.sunknowledge.dme.rcm.repository.primaryclaimrepository.ClaimsSubmissionMasterRepo;
import com.sunknowledge.dme.rcm.repository.primaryclaimrepository.ServiceLinesMasterRepo;
import com.sunknowledge.dme.rcm.validation.DmeUtilities;

@Primary
@Service
public class PrimaryClaimSubmissionServiceImpl implements PrimaryClaimSubmissionService {
	...
	@Autowired
	private ClaimsSubmissionMasterRepo claimSubmissionRepository;
	@Autowired
	private ServiceLinesMasterRepo primaryServiceLineRepo;
	@Autowired
	...
	public ServiceOutcome<ResultClaimSubmissionOutcome> accessProfessionalClaimsSubmission(String soId, String claimControlNumber) {
		...
		ClaimSubmissionStatus objvalidateClaimSubmission = claimSubmissionStatusRepo.getClaimSubmissionValidation(Long.valueOf(soId),"P");
		System.out.println("ClaimSubmissionStatus      "+objvalidateClaimSubmission);
		ClaimsSubmissionMaster claims = claimSubmissionRepository.getPrimarySubmissionDetails(Long.valueOf(soId), objvalidateClaimSubmission.getIntClaimNo());
		List<ServiceLinesMaster> serviceLines = primaryServiceLineRepo
				.getPrimarySubmissionServiceDetails(claims.getChangeHealthPrimarySubmisionMasterId());
		...
		if (!isAfter) {
			...
			try {
				...
				if (professionalClaimHealthCheck(token).getOutcome()) {
					...
					System.out.println("Internal Claim Control Number===== "+claims.getClaimControlNo());
					objClaimSubmissionStatus = claimSubmissionStatusRepo
							.getClaimSubmissionStatus(claims.getClaimControlNo());
					System.out.println("Internal Claim Control Number after "+objClaimSubmissionStatus.getIntClaimNo());
	...
	ClaimSubmissionInput setdata(ClaimsSubmissionMaster claims, List<ServiceLinesMaster> serviceLines) {
