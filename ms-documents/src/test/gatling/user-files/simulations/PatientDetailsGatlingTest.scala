...
/**
 * Performance test for the PatientDetails entity.
 */
class PatientDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PatientDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all patientDetails")
            .get("/services/documents/api/patient-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new patientDetails")
            .post("/services/documents/api/patient-details")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "patientFname":"SAMPLE_TEXT"
                , "patientLname":"SAMPLE_TEXT"
                ...
                , "mrno":"SAMPLE_TEXT"
                , "dateTime":"SAMPLE_TEXT"
                , "status":"0"
                , "isTagged":null
                , "documentPath":"SAMPLE_TEXT"
                , "qrCodeStatus":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_patientDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created patientDetails")
                .get("/services/documents${new_patientDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created patientDetails")
            .delete("/services/documents${new_patientDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
