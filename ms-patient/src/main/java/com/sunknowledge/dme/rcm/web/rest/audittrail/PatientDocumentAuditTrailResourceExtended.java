...
@RestController
@RequestMapping("/api")
public class PatientDocumentAuditTrailResourceExtended {
    private final Logger log = LoggerFactory.getLogger(PatientDocumentAuditTrailResourceExtended.class);

    @Value("${jhipster.clientApp.name}")
    private String applicationName;

    @Autowired
    @Qualifier("patientDocumentAuditTrailServiceExtendedImpl")
    PatientDocumentAuditTrailServiceExtended patientDocumentAuditTrailServiceExtended;

    /**
     * Get Patient Audit Trail .......
     */
    @GetMapping("getPatientDocumentAuditLog")
    public Flux<AuditLogReportDTO> getPatientDocumentAuditLog(
        @ModelAttribute AuditLogParameterDTO auditLogParameterDTO
    ) {
        return patientDocumentAuditTrailServiceExtended.getPatientDocumentAuditLog(auditLogParameterDTO);
    }
}
