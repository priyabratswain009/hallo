...
/**
 * Performance test for the FunctionalAbility entity.
 */
class FunctionalAbilityGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the FunctionalAbility entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all functionalAbilities")
            .get("/services/patient/api/functional-abilities")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new functionalAbility")
            .post("/services/patient/api/functional-abilities")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "functionalAbilityCode":"SAMPLE_TEXT"
                , "functionalAbilityName":"SAMPLE_TEXT"
                ...
                , "functionalAbilityUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_functionalAbility_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created functionalAbility")
                .get("/services/patient${new_functionalAbility_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created functionalAbility")
            .delete("/services/patient${new_functionalAbility_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
