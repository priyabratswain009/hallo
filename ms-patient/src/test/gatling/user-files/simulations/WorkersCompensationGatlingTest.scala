...
/**
 * Performance test for the WorkersCompensation entity.
 */
class WorkersCompensationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the WorkersCompensation entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all workersCompensations")
            .get("/services/patient/api/workers-compensations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new workersCompensation")
            .post("/services/patient/api/workers-compensations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "patientId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_workersCompensation_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created workersCompensation")
                .get("/services/patient${new_workersCompensation_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created workersCompensation")
            .delete("/services/patient${new_workersCompensation_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
