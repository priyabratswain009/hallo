...
/**
 * Performance test for the PatientDoctorMap entity.
 */
class PatientDoctorMapGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PatientDoctorMap entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all patientDoctorMaps")
            .get("/services/patient/api/patient-doctor-maps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new patientDoctorMap")
            .post("/services/patient/api/patient-doctor-maps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "patientId":"0"
                , "createdById":"0"
                ...
                , "patientDoctorMapUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_patientDoctorMap_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created patientDoctorMap")
                .get("/services/patient${new_patientDoctorMap_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created patientDoctorMap")
            .delete("/services/patient${new_patientDoctorMap_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
