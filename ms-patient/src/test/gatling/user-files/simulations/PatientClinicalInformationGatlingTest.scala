...
/**
 * Performance test for the PatientClinicalInformation entity.
 */
class PatientClinicalInformationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PatientClinicalInformation entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all patientClinicalInformations")
            .get("/services/patient/api/patient-clinical-informations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new patientClinicalInformation")
            .post("/services/patient/api/patient-clinical-informations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "height":"0"
                , "weight":"0"
                ...
                , "patientClinicalInformationUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_patientClinicalInformation_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created patientClinicalInformation")
                .get("/services/patient${new_patientClinicalInformation_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created patientClinicalInformation")
            .delete("/services/patient${new_patientClinicalInformation_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
