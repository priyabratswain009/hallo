...
/**
 * Performance test for the PatientDiagnosis entity.
 */
class PatientDiagnosisGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PatientDiagnosis entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all patientDiagnoses")
            .get("/services/patient/api/patient-diagnoses")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new patientDiagnosis")
            .post("/services/patient/api/patient-diagnoses")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "patientId":"0"
                , "diagnosisCodeType":"SAMPLE_TEXT"
                , "diagnosisCode":"SAMPLE_TEXT"
                , "diagnosisDescription":"SAMPLE_TEXT"
                ...
                , "patientDiagnosisUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_patientDiagnosis_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created patientDiagnosis")
                .get("/services/patient${new_patientDiagnosis_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created patientDiagnosis")
            .delete("/services/patient${new_patientDiagnosis_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
