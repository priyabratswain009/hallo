...
@RestController
@RequestMapping("/api")
public class PriceDetailsAuditLogResourceExtended {
    private final Logger log = LoggerFactory.getLogger(PriceDetailsAuditLogResourceExtended.class);

    @Value("${jhipster.clientApp.name}")
    private String applicationName;

    @Autowired
    @Qualifier("priceDetailsAuditLogServiceExtendedImpl")
    PriceDetailsAuditLogServiceExtended priceDetailsAuditLogServiceExtended;

    /**
     * Get PriceDetails Audit Trail .......
     */
    @GetMapping("getPriceDetailsAuditLog")
    public List<AuditLogReportDTO> getPriceDetailsAuditLog(
        @ModelAttribute ItemNoAuditLogParameterDTO auditLogParameterDTO
    ) {
        return priceDetailsAuditLogServiceExtended.getPriceDetailsAuditLog(auditLogParameterDTO);
    }
}
