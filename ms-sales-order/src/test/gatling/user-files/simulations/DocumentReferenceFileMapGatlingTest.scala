...
/**
 * Performance test for the DocumentReferenceFileMap entity.
 */
class DocumentReferenceFileMapGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DocumentReferenceFileMap entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all documentReferenceFileMaps")
            .get("/services/salesorder/api/document-reference-file-maps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new documentReferenceFileMap")
            .post("/services/salesorder/api/document-reference-file-maps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "checklistId":"0"
                ...
                , "documentReferenceFileMapUuid":null
                , "fileReference":"SAMPLE_TEXT"
                , "documentReferenceNotes":"SAMPLE_TEXT"
                , "soId":"0"
                , "itemGroupId":"0"
                , "itemGroupName":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_documentReferenceFileMap_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created documentReferenceFileMap")
                .get("/services/salesorder${new_documentReferenceFileMap_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created documentReferenceFileMap")
            .delete("/services/salesorder${new_documentReferenceFileMap_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
