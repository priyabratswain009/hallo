...
/**
 * Performance test for the ParSoMap entity.
 */
class ParSoMapGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ParSoMap entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all parSoMaps")
            .get("/services/salesorder/api/par-so-maps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new parSoMap")
            .post("/services/salesorder/api/par-so-maps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "parId":"0"
                , "parNo":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_parSoMap_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created parSoMap")
                .get("/services/salesorder${new_parSoMap_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created parSoMap")
            .delete("/services/salesorder${new_parSoMap_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
