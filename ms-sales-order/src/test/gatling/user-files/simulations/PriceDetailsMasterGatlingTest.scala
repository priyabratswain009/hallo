...
/**
 * Performance test for the PriceDetailsMaster entity.
 */
class PriceDetailsMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PriceDetailsMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all priceDetailsMasters")
            .get("/services/salesorder/api/price-details-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new priceDetailsMaster")
            .post("/services/salesorder/api/price-details-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "priceTableId":"0"
                , "itemId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_priceDetailsMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created priceDetailsMaster")
                .get("/services/salesorder${new_priceDetailsMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created priceDetailsMaster")
            .delete("/services/salesorder${new_priceDetailsMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
