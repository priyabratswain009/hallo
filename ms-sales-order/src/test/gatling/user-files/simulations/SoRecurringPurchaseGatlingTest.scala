...
/**
 * Performance test for the SoRecurringPurchase entity.
 */
class SoRecurringPurchaseGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SoRecurringPurchase entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all soRecurringPurchases")
            .get("/services/salesorder/api/so-recurring-purchases")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new soRecurringPurchase")
            .post("/services/salesorder/api/so-recurring-purchases")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "soId":"0"
                , "itemId":"0"
                ...
                , "qty":"0"
                , "billingInterval":"0"
                , "initialDos":"2020-01-01T00:00:00.000Z"
                , "period":"0"
                , "status":"SAMPLE_TEXT"
                ...
                , "updatedById":"0"
                , "soRecurringPurchaseUuid":null
                , "createdByName":"SAMPLE_TEXT"
                , "updatedByName":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_soRecurringPurchase_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created soRecurringPurchase")
                .get("/services/salesorder${new_soRecurringPurchase_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created soRecurringPurchase")
            .delete("/services/salesorder${new_soRecurringPurchase_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
