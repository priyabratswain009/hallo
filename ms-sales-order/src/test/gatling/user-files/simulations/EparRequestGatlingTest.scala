...
/**
 * Performance test for the EparRequest entity.
 */
class EparRequestGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the EparRequest entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all eparRequests")
            .get("/services/salesorder/api/epar-requests")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new eparRequest")
            .post("/services/salesorder/api/epar-requests")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "soId":"0"
                , "soNo":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_eparRequest_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created eparRequest")
                .get("/services/salesorder${new_eparRequest_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created eparRequest")
            .delete("/services/salesorder${new_eparRequest_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
