...
/**
 * Performance test for the DmeGroupChecklistMaster entity.
 */
class DmeGroupChecklistMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DmeGroupChecklistMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all dmeGroupChecklistMasters")
            .get("/services/salesorder/api/dme-group-checklist-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new dmeGroupChecklistMaster")
            .post("/services/salesorder/api/dme-group-checklist-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "dmeGroupId":"0"
                , "dmeGroupName":"SAMPLE_TEXT"
                , "checklistId":"0"
                ...
                , "dmeGroupChecklistMasterUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_dmeGroupChecklistMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created dmeGroupChecklistMaster")
                .get("/services/salesorder${new_dmeGroupChecklistMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created dmeGroupChecklistMaster")
            .delete("/services/salesorder${new_dmeGroupChecklistMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
