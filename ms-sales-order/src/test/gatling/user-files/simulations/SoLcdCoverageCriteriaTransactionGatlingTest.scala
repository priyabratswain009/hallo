...
/**
 * Performance test for the SoLcdCoverageCriteriaTransaction entity.
 */
class SoLcdCoverageCriteriaTransactionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SoLcdCoverageCriteriaTransaction entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all soLcdCoverageCriteriaTransactions")
            .get("/services/salesorder/api/so-lcd-coverage-criteria-transactions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new soLcdCoverageCriteriaTransaction")
            .post("/services/salesorder/api/so-lcd-coverage-criteria-transactions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "soId":"0"
                , "checklistId":"0"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created soLcdCoverageCriteriaTransaction")
                .get("/services/salesorder${new_soLcdCoverageCriteriaTransaction_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created soLcdCoverageCriteriaTransaction")
            .delete("/services/salesorder${new_soLcdCoverageCriteriaTransaction_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
