...
/**
 * Performance test for the SecondaryClaimSubmisionServicelines entity.
 */
class SecondaryClaimSubmisionServicelinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SecondaryClaimSubmisionServicelines entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all secondaryClaimSubmisionServicelines")
            .get("/services/salesorder/api/secondary-claim-submision-servicelines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new secondaryClaimSubmisionServicelines")
            .post("/services/salesorder/api/secondary-claim-submision-servicelines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "changeHealthSecondarySubmisionMasterId":"0"
                , "originalDos":"2020-01-01T00:00:00.000Z"
                , "dosTo":"2020-01-01T00:00:00.000Z"
                ...
                , "claimServicelineControlNo":"SAMPLE_TEXT"
                ...
                , "lineAdjustment":"SAMPLE_TEXT"
                , "updatedById":"SAMPLE_TEXT"
                , "updatedDate":"2020-01-01T00:00:00.000Z"
                , "updatedByName":"SAMPLE_TEXT"
                , "secondaryClaimSubmisionServicelinesUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_secondaryClaimSubmisionServicelines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created secondaryClaimSubmisionServicelines")
                .get("/services/salesorder${new_secondaryClaimSubmisionServicelines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created secondaryClaimSubmisionServicelines")
            .delete("/services/salesorder${new_secondaryClaimSubmisionServicelines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
