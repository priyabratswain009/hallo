...
/**
 * Performance test for the MemberElligibility entity.
 */
class MemberElligibilityGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the MemberElligibility entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all memberElligibilities")
            .get("/services/salesorder/api/member-elligibilities")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new memberElligibility")
            .post("/services/salesorder/api/member-elligibilities")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "salesOrderId":"0"
                ...
                , "status":"2020-01-01T00:00:00.000Z"
                , "createdById":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_memberElligibility_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created memberElligibility")
                .get("/services/salesorder${new_memberElligibility_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created memberElligibility")
            .delete("/services/salesorder${new_memberElligibility_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
