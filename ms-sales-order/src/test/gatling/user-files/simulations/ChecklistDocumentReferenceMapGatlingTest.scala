...
/**
 * Performance test for the ChecklistDocumentReferenceMap entity.
 */
class ChecklistDocumentReferenceMapGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ChecklistDocumentReferenceMap entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all checklistDocumentReferenceMaps")
            .get("/services/salesorder/api/checklist-document-reference-maps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new checklistDocumentReferenceMap")
            .post("/services/salesorder/api/checklist-document-reference-maps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "checklistId":"0"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created checklistDocumentReferenceMap")
                .get("/services/salesorder${new_checklistDocumentReferenceMap_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created checklistDocumentReferenceMap")
            .delete("/services/salesorder${new_checklistDocumentReferenceMap_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
