...
/**
 * Performance test for the InvoiceMasterDetails entity.
 */
class InvoiceMasterDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the InvoiceMasterDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all invoiceMasterDetails")
            .get("/services/salesorder/api/invoice-master-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new invoiceMasterDetails")
            .post("/services/salesorder/api/invoice-master-details")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "invoiceNo":"SAMPLE_TEXT"
                , "invoiceDate":"2020-01-01T00:00:00.000Z"
                , "invoiceToEntity":"SAMPLE_TEXT"
                , "invoiceToPayorId":"0"
                , "invoiceToPayorName":"SAMPLE_TEXT"
                , "salesOrderId":"0"
                ...
                , "invoiceMasterDetailsUuid":null
                , "invoiceStatus":"SAMPLE_TEXT"
                , "primarySubmissionMasterId":"0"
                , "claimControlNo":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_invoiceMasterDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created invoiceMasterDetails")
                .get("/services/salesorder${new_invoiceMasterDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created invoiceMasterDetails")
            .delete("/services/salesorder${new_invoiceMasterDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
