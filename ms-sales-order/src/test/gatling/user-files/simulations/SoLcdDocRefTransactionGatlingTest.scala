...
/**
 * Performance test for the SoLcdDocRefTransaction entity.
 */
class SoLcdDocRefTransactionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SoLcdDocRefTransaction entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all soLcdDocRefTransactions")
            .get("/services/salesorder/api/so-lcd-doc-ref-transactions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new soLcdDocRefTransaction")
            .post("/services/salesorder/api/so-lcd-doc-ref-transactions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "soId":"0"
                , "itemId":"0"
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created soLcdDocRefTransaction")
                .get("/services/salesorder${new_soLcdDocRefTransaction_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created soLcdDocRefTransaction")
            .delete("/services/salesorder${new_soLcdDocRefTransaction_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
