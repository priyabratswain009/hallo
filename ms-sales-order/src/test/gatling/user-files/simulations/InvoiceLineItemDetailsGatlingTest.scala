...
/**
 * Performance test for the InvoiceLineItemDetails entity.
 */
class InvoiceLineItemDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the InvoiceLineItemDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all invoiceLineItemDetails")
            .get("/services/salesorder/api/invoice-line-item-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new invoiceLineItemDetails")
            .post("/services/salesorder/api/invoice-line-item-details")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                ...
                , "invoiceLineItemDetailsUuid":null
                , "secondaryInvoiceNo":"SAMPLE_TEXT"
                , "tertiaryInvoiceNo":"SAMPLE_TEXT"
                , "patientInvoiceNo":"SAMPLE_TEXT"
                , "primaryInvoiceId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_invoiceLineItemDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created invoiceLineItemDetails")
                .get("/services/salesorder${new_invoiceLineItemDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created invoiceLineItemDetails")
            .delete("/services/salesorder${new_invoiceLineItemDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
