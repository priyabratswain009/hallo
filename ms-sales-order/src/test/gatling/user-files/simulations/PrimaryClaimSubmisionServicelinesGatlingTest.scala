...
/**
 * Performance test for the PrimaryClaimSubmisionServicelines entity.
 */
class PrimaryClaimSubmisionServicelinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PrimaryClaimSubmisionServicelines entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all primaryClaimSubmisionServicelines")
            .get("/services/salesorder/api/primary-claim-submision-servicelines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new primaryClaimSubmisionServicelines")
            .post("/services/salesorder/api/primary-claim-submision-servicelines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                ...
                , "primaryClaimSubmisionServicelinesUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_primaryClaimSubmisionServicelines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created primaryClaimSubmisionServicelines")
                .get("/services/salesorder${new_primaryClaimSubmisionServicelines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created primaryClaimSubmisionServicelines")
            .delete("/services/salesorder${new_primaryClaimSubmisionServicelines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
