...
/**
 * Performance test for the PrimaryClaimResubmisionMaster entity.
 */
class PrimaryClaimResubmisionMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PrimaryClaimResubmisionMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all primaryClaimResubmisionMasters")
            .get("/services/salesorder/api/primary-claim-resubmision-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new primaryClaimResubmisionMaster")
            .post("/services/salesorder/api/primary-claim-resubmision-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "salesOrderId":"0"
                ...
                , "primaryClaimResubmisionMasterUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_primaryClaimResubmisionMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created primaryClaimResubmisionMaster")
                .get("/services/salesorder${new_primaryClaimResubmisionMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created primaryClaimResubmisionMaster")
            .delete("/services/salesorder${new_primaryClaimResubmisionMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
