...
/**
 * Performance test for the EfaxResponse entity.
 */
class EfaxResponseGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the EfaxResponse entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all efaxResponses")
            .get("/services/salesorder/api/efax-responses")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new efaxResponse")
            .post("/services/salesorder/api/efax-responses")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "efaxReceivedDate":"2020-01-01T00:00:00.000Z"
                , "senderEmail":"SAMPLE_TEXT"
                , "emailSubjectLine":"SAMPLE_TEXT"
                , "attachmentName":"SAMPLE_TEXT"
                , "isQrDecoded":null
                , "qrValue":"SAMPLE_TEXT"
                , "isCmn":null
                , "isPar":null
                ...
                , "soId":"0"
                , "soNo":"SAMPLE_TEXT"
                , "isManuallyTagged":null
                , "isPatientRecord":null
                , "efaxNumber":"SAMPLE_TEXT"
                , "isPoAck":null
                , "documentRenameTo":"SAMPLE_TEXT"
                , "createdById":"0"
                ...
                , "status":"SAMPLE_TEXT"
                , "efaxResponseUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_efaxResponse_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created efaxResponse")
                .get("/services/salesorder${new_efaxResponse_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created efaxResponse")
            .delete("/services/salesorder${new_efaxResponse_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
