...
/**
 * Performance test for the HcpcsDmeGroupMaster entity.
 */
class HcpcsDmeGroupMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the HcpcsDmeGroupMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all hcpcsDmeGroupMasters")
            .get("/services/salesorder/api/hcpcs-dme-group-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new hcpcsDmeGroupMaster")
            .post("/services/salesorder/api/hcpcs-dme-group-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                ...
                , "hcpcsDmeGroupMasterUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_hcpcsDmeGroupMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created hcpcsDmeGroupMaster")
                .get("/services/salesorder${new_hcpcsDmeGroupMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created hcpcsDmeGroupMaster")
            .delete("/services/salesorder${new_hcpcsDmeGroupMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
