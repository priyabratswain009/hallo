...
/**
 * Performance test for the PayerMaster entity.
 */
class PayerMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PayerMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all payerMasters")
            .get("/services/salesorder/api/payer-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new payerMaster")
            .post("/services/salesorder/api/payer-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "payerId":"SAMPLE_TEXT"
                ...
                , "payerMasterUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_payerMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created payerMaster")
                .get("/services/salesorder${new_payerMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created payerMaster")
            .delete("/services/salesorder${new_payerMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
