...
/**
 * Performance test for the ParDetails entity.
 */
class ParDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ParDetails entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all parDetails")
            .get("/services/salesorder/api/par-details")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new parDetails")
            .post("/services/salesorder/api/par-details")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "parId":"0"
                , "itemId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_parDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created parDetails")
                .get("/services/salesorder${new_parDetails_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created parDetails")
            .delete("/services/salesorder${new_parDetails_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
