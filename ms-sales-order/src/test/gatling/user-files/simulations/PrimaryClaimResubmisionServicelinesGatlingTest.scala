...
/**
 * Performance test for the PrimaryClaimResubmisionServicelines entity.
 */
class PrimaryClaimResubmisionServicelinesGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PrimaryClaimResubmisionServicelines entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all primaryClaimResubmisionServicelines")
            .get("/services/salesorder/api/primary-claim-resubmision-servicelines")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new primaryClaimResubmisionServicelines")
            .post("/services/salesorder/api/primary-claim-resubmision-servicelines")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "changeHealthPrimaryResubmisionMasterId":"0"
                ...
                , "qty":"0"
                , "insertedById":"0"
                , "nsertedDate":"2020-01-01T00:00:00.000Z"
                , "updatedById":"0"
                ...
                , "primaryClaimResubmisionServicelinesUuid":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_primaryClaimResubmisionServicelines_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created primaryClaimResubmisionServicelines")
                .get("/services/salesorder${new_primaryClaimResubmisionServicelines_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created primaryClaimResubmisionServicelines")
            .delete("/services/salesorder${new_primaryClaimResubmisionServicelines_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
