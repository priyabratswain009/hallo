...
/**
 * Performance test for the CoverageCriteriaFileMap entity.
 */
class CoverageCriteriaFileMapGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CoverageCriteriaFileMap entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all coverageCriteriaFileMaps")
            .get("/services/salesorder/api/coverage-criteria-file-maps")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new coverageCriteriaFileMap")
            .post("/services/salesorder/api/coverage-criteria-file-maps")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "checklistId":"0"
                , "checklistName":"SAMPLE_TEXT"
                , "documentReferenceId":"0"
                , "documentReferenceName":"SAMPLE_TEXT"
                ...
                , "coverageCriteriaFileMapUuid":null
                , "fileReference":"SAMPLE_TEXT"
                , "coverageCriteriaNotes":"SAMPLE_TEXT"
                , "soId":"0"
                , "itemGroupId":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_coverageCriteriaFileMap_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created coverageCriteriaFileMap")
                .get("/services/salesorder${new_coverageCriteriaFileMap_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created coverageCriteriaFileMap")
            .delete("/services/salesorder${new_coverageCriteriaFileMap_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
